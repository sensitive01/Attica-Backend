...
// const addProduct = async (req, res) => {
//   try {
//     const { title, items, unitofmeasurement, sku, description, image, stock, categories, category, sales, prices } = req.body;

//     // Check if 'items' exists in the request body before mapping over it
//     let formattedItems = [];
//     if (items) {
...
const deleteManyProducts = async (req, res) => {
  try {
    // Validate the input data to prevent any unexpected behavior
    if (!Array.isArray(req.body.ids) || req.body.ids.length === 0) {
      return res.status(400).send({ message: "Invalid IDs provided for deletion" });
    }

    const cname = req.cname;

    // Delete multiple products based on the provided IDs
