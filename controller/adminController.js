...
const { signInToken, tokenForVerify, sendEmail, generateExecutiveToken } = require('../config/auth');
...
const loginAdmin = async (req, res) => {
  try {
    ...
    let redirectToDashboard = true; // Default to true
    let redirectURL = '/dashboard'; // Default redirect URL for regular admins
    if (admin.role === "Executive") { // Fixed capitalization in role check
      token = generateExecutiveToken(admin);
      redirectURL = '/executive-dashboard'; // Change redirect URL for executive admins
    } else {
      token = signInToken(admin);
    }
    res.send({ token, ...userData, redirectToDashboard, redirectURL });
  } catch (err) {
    console.error(err);
...
const forgetPassword = async (req, res) => {
  ...
  if (!isAdded) {
    ...
    const body = {
      ...
      <p>A request has been received to change the password for your <strong>Attica</strong> account </p>

        <p>This link will expire in <strong> 5 minute</strong>.</p>
        ...
        <p style="margin-top: 35px;">If you did not initiate this request, please contact us immediately at support@Attica.com</p>

        <p style="margin-bottom:0px;">Thank you</p>
        <strong>Attica Team</strong>
             `,
    };
